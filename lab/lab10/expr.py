...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError("Not found {0} in current environment".format(self.string))
...
class LambdaExpr(Expr):
    ...
    The `parameters` attribute is a list of variable tnames (a list of strings).
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        cur_operator = self.operator.eval(env)
        cur_operands = [operand.eval(env) for operand in self.operands]
