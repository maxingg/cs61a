...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    rest_aux = 6 - start - end

    if n == 1:
        print_move(start, end)
    else:
        move_stack(n-1, start, rest_aux)
        print_move(start, end)
        move_stack(n-1, rest_aux, end)
...
class Account:
    ...
    def time_to_retire(self, amount):
        ...
        while balance_copy < amount:
            balance_copy = balance_copy * (1 + self.interest)
            count += 1
        return count
...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    wrong_password, count = [], 0
    def withdraw(amount, input_password):
        nonlocal balance, count
        if count > 2:
            return "Your account is locked. Attempts: {0}".format(wrong_password)
        else:
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint_withdraw(amount, input_password):
        # 这一层只能验证这两个密码，调用到下一层只有old_password可用（越往下走可用的密码选项越少）
